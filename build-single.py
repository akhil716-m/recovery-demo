...
(function(){
  ...
  function addExitBtn(){
    ...
    exit.onclick = function(){ localStorage.removeItem('rrSetupV2'); try{ sessionStorage.removeItem('rrInApp'); }catch(e){} try{ sessionStorage.removeItem('rrWizardProgress'); }catch(e){} location.reload(); };
  ...
  // Show the landing page on every load -- including a plain refresh. The
  // dashboard is reached only via the one page load that immediately follows
  // finishing (or skipping) onboarding: that transition sets rrInApp and
  // reloads, and this flag is consumed (removed) the instant it's read, so
  // it only ever satisfies that single load. Any load after that -- another
  // refresh, reopening the tab, pasting the link again -- finds no flag and
  // falls through to the landing page below.
  var _justEntered = (function(){
    try{
      var v = sessionStorage.getItem('rrInApp')==='1';
      if(v) sessionStorage.removeItem('rrInApp');
      return v;
    }catch(e){ return false; }
  })();
  if(localStorage.getItem('rrSetupV2') && _justEntered){
  ...
  // Persist in-progress wizard state (current step + entered fields) per tab
  // so a reload mid-onboarding resumes exactly where the user left off,
  // instead of dropping them back at the landing page. Cleared once setup
  // finishes/is skipped (state.js) or the user exits back to landing.
  function saveProgress(){
    if(stepIndex === -1){ clearProgress(); return; }
    try{ sessionStorage.setItem('rrWizardProgress', JSON.stringify({stepIndex:stepIndex, state:state})); }catch(e){}
  }
  function clearProgress(){
    try{ sessionStorage.removeItem('rrWizardProgress'); }catch(e){}
  }
  (function restoreProgress(){
    try{
      var raw = sessionStorage.getItem('rrWizardProgress');
      if(!raw) return;
      var saved = JSON.parse(raw);
      if(saved && typeof saved.stepIndex === 'number' && saved.stepIndex > -1 && saved.stepIndex < STEPS.length){
        stepIndex = saved.stepIndex;
        if(saved.state) for(var k in saved.state) state[k] = saved.state[k];
      }
    }catch(e){}
  })();
  ...
  function renderWizard(){
    ...
    var backBtn = btn('','display:flex;align-items:center;justify-content:center;width:30px;height:30px;border-radius:8px;border:none;background:none;color:'+TEXT+';cursor:pointer',
      function(){ stepIndex > 0 ? (stepIndex--, render()) : (clearProgress(), stepIndex=-1, render()); });
  ...
  function renderStep(outerContainer){
    ...
    function inputEl(val, placeholder, type, onchange){
      ...
      i.oninput = function(){ onchange(i.value); saveProgress(); };
      return i;
    }
...
(function(){
  ...
  // A single fixed-delay setTimeout here used to be a race: if React hadn't
  // mounted the nav bar by the 300ms mark (slower device, cold/no-store
  // load, etc.) simBtn stayed null and Live Flow silently never got added
  // for that page view, with nothing ever retrying it. Watch for the nav
  // bar the same way the other injected fixups in this file do, so it
  // self-heals regardless of how long hydration takes.
  if(localStorage.getItem('rrSetupV2')){
    (function watchSimNav(){
      var nav = document.querySelector('nav');
      if(!nav){ setTimeout(watchSimNav, 100); return; }
      var t; new MutationObserver(function(){ clearTimeout(t); t = setTimeout(window.rrAddSimulationNav, 60); })
        .observe(nav, {childList:true, subtree:true});
      window.rrAddSimulationNav();
    })();
    setTimeout(window.rrAddSimulationNav, 300);
    setTimeout(window.rrAddSimulationNav, 800);
    setTimeout(window.rrAddSimulationNav, 1500);
  }
})();
</script>"""
